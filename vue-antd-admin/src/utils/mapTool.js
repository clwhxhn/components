...
export const iconAnimation = (params) => {
  const { map, features, layer, colors, key, speed = 1, radiusNum = 16, width = 6,warnList =[] } = params;
  ...
  layer.on('postrender', (event) => {
    ...
    // 外部圈
    const pointStyle = (warnType) => {
      return new Style({
        image: new Circle({
          radius: radius,
          stroke: new Stroke({
            //颜色可以根据业务需求改为变量
            color: `rgba(${colors[warnType]}, ${opacity})`,
            width: width,
          }),
    ...
    // 内部圈
    const innerPointStyle = (warnType) => {
      return new Style({
        image: new Circle({
          radius: radius,
          stroke: new Stroke({
    ...
    features.forEach((fea) => {
      // console.log(fea,'fea')
      const obj =  warnList.find(i=>i.stCode === fea.values_.stCode)
      // const properties = fea.get('properties');
      //一圈
      vectorContext.drawFeature(fea, pointStyle(Number(obj.warnLevel)));
      //内部加一个圈
      vectorContext.drawFeature(fea, innerPointStyle(Number(obj.warnLevel)));
    });
  });
  
  /*
  // 废弃原因：大量的点位预警，使用postcompose事件会导致地图渲染频率过高，导致动画效果不流畅
   map.mapInstance.on('postcompose', () => {
    //控制频率，增量越大，速度越快
    radius = radius + speed;
    //渲染map 触发layer的postrender方法
    map.mapInstance.render();
  }); */
  // 用setTimeout实现动画效果，解决postcompose事件导致地图渲染频率过高，导致动画效果不流畅
  const animate = () => {
    radius += speed;
    if (radius > radiusNum) {
      radius = 0;
