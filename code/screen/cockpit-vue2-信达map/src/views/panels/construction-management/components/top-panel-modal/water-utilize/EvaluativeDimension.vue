...
import { getEvaluativeDimensionWaterUtilize } from "../../../mock/mock";
...
export default {
  ...
  async mounted() {
    ...
    this.list = await getEvaluativeDimensionWaterUtilize();
    this.renderChart();
  },
  //   watch: {
  ...
  methods: {
      ...
      const options = {
        tooltip: {
          ...
          formatter: function (params) {
            // 判断是否为 NaN
            const value = isNaN(params.value) ? "" : `${params.value}%`;
        ...
        series: [
          {
            ...
            data: cq.features.map((it) => {
              ...
              const value = item ? item.val : 0; // Math.round(Math.random() * 7);
              return {
    ...
    getColorByValue(val) {
      console.log(val, "val9090");
      if (!val) return this.colorList[4].color;
      if (val >= 40) return this.colorList[0].color;
      if (val >= 30 && val < 40) return this.colorList[1].color;
      if (val >= 20 && val < 3) return this.colorList[2].color;
      if (val >= 10 && val < 20) return this.colorList[3].color;
