...
import * as optionLight from '@/components/waterLevelFlowChart/chartOptionsLight'
...
export function getSeriesMarker(cfgList, seriesName, theme) {
  ...
  const opt = cfgList.find(item => seriesName.indexOf(item.name) >= 0)
  if (opt && opt.color) {
    ...
    axisPointer: {
     ...
     formatter: function (params) {
        const htmlStr = params
          .map(item => {
            return `<div>${getSeriesMarker([], item.seriesName, 'light')}
              <span>${item.seriesName}：</span>
              <span>${item.value[1]}${getSeriesUnit(item.seriesName)}</span>
            </div>`
          })
          .join('')
        return `<div class="tooltip-box light"><div>${params[0].name}</div>${htmlStr}</div>`
      }
  },
  grid: [
    {
        ...
        itemStyle: {
          color: {
            ...
            colorStops: [
              {
                offset: 0,
                color: "rgba(225, 244, 231, 0.5)",
              },
              {
                offset: 1,
                color: "rgba(225, 244, 231, 0)",
              },
            ],
          },
        },
